# hostel_booking: Secure Hostel Booking on Sui
...
### Function Structure:
The hostel_booking module offers various functionalities categorized as follows:

**1. Institution Management:**

`create_institution(ctx: &mut TxContext, name: String)`: Creates a new Institution object on the Sui blockchain, storing its name, student fee table, balance, and address.

`get_institution_balance(institution: &Institution) : &Balance<SUI>`: Retrieves the current balance of a specific Institution.

`withdraw_funds(institution: &mut Institution, amount: u64, ctx: &mut TxContext)`: Allows authorized personnel to withdraw funds from the Institution's balance.

**2. Student Management:**

`create_student(ctx: &mut TxContext, name: String, institution_address: address)`: Creates a new Student object linked to a specific Institution.

`top_up_student_balance(student: &mut Student, amount: Coin<SUI>, ctx: &mut TxContext)`: Enables students to add funds to their Sui account for booking purposes.

**3. Room Management and Booking:**

`create_room_memo(institution: &mut Institution, semester_payment: u64, student_fee: u64, ... , ctx: &mut TxContext)`: Creates a RoomMemo object within an Institution, specifying student fees, semester payments, and room details.

`book_room(institution: &mut Institution, student: &mut Student, room: &mut HostelRoom, room_memo_id: ID, clock: &Clock, ctx: &mut TxContext)`: Handles the core booking process. Students can book a room by referencing a valid RoomMemo, deducting the corresponding fee from their balance and adding it to the Institution's balance.

`top_up_institution_balance(institution: &mut Institution, student: &mut Student, room: &mut HostelRoom, room_memo_id: ID, clock: &Clock, ctx: &mut TxContext)`: This function simplifies fee payment by calling book_room internally. It can only be called by a student.

`return_room(institution: &mut Institution, student: &mut Student, room: &mut HostelRoom, ctx: &mut TxContext)`: Allows students to return a previously booked room, increasing the available beds in that room.

**4. Ownership Transfer:**

`transfer_room_ownership(student: &Student, room: HostelRoom) (entry function)`: This function enables transferring the ownership of a room to the student 
